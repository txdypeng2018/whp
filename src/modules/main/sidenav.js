...
(function(app) {
  ...
  var ctrl = function($scope, $state, $location) {
    var simpleNodes = [
      { id:1, pId:0, name:'父节点1 - 展开', open:true, state: 'main.resourcesList'},
      ...
      { id: 5, pId: 0, name: 'iframe node'},
      { id: 51, pId: 5, name: 'iframe node 1', url: 'http://localhost:8081'},
      { id: 52, pId: 5, name: 'iframe node 2', url: 'http://localhost:8082'}
    ];
    ...
    $scope.treeOptions = {
      ...
      onClick: function (node) {
        ...
        if (node.state) {
          ...
          $location.url('/playground/iframe?url=' + node.url);
        }
      }
    };
  };

  app.controller('sidenavCtrl', ctrl);
